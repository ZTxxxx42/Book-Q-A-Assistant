...
6. **`BGE_FORCE_CPU=1`** forces bge to CPU to avoid CUDA context conflicts with Ollama/vLLM sharing the GPU. Currently set in `.env` due to a TDR/GPU corruption issue; remove after a reboot if running a cloud LLM with no local Ollama.
